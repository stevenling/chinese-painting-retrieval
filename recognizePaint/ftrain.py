...
ss = StratifiedShuffleSplit(n_splits=1, test_size=0.2)#10个数据 8个是训练数据，2个是测试数据

train_idx, val_idx = next(ss.split(codes, labels))
#训练集索引 以及标签索引
#print(len(train_idx))
#总共2400条数据 切成 1920 240 240
#train_idx 有1920
#val_idx 有 480 条
#half_val_len 有 240
...
print("Train shapes (x, y):", train_x.shape, train_y.shape) #训练集数量 维度 标签数量 维度
...
假设我们使用一个 256 维的全连接层，
一个 3 维的全连接层（因为我们要分类3种不同类的国画），
和一个 softmax 层。当然，这里的网络结构可以任意修改，尝试其他的结构以找到合适的结构。
'''
# 输入数据的维度

# 标签数据的维度
labels_ = tf.placeholder(tf.int64, shape=[None, labels_vecs.shape[1]])#3维
