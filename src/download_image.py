...
def download_page(url):
    """
    使用自定义 User-Agent 伪装成浏览器，获取指定 URL 的页面内容。
    """
...
def get_all_first_url():
    """
    爬取国画列表页中的所有作品详情页 URL，并保存到全局列表 all_url_list 中。
    通过翻页按钮“下一页”迭代更新 temp_url，直到遍历完指定页数。
    """
    # 花鸟类
    temp_url = f"{config.BASE_URL}/works/guohua.html?attributNum=797&attributNam=%E9%A2%98%E6%9D%90&caizhistr=796%2C%E9%A2%98%E6%9D%90%2C"
    # 页数
    page_index = 0
    while page_index < 1:
        ...
        for all_pic_index in all_pic:
            ...
            real_all_href = all_href['href']  # 拿到url
        ...
        for next_btn in next_btn_list:
            ...
            if next_btn.getText() == "下一页" and next_btn.get("href") is not None:
                temp_url = config.BASE_URL + next_btn.get("href")
        # print(temp_url)
